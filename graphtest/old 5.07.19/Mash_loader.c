...
//возвращает true, если файл еще не закночилс€
{
...
{//начинаетс€ ли строка a со строки b
    while ((*b!='\0')&&(*a!='\0')&&(*a==*b))
    {
        a++;
        b++;
...
{//копирует в строку to начало строки from до первого вхождени€ символа
 //splitter или конца строки.
 // ¬озвращает ссылку на следующий после него элемент
...
{//переводит строку в целое беззнаковое число,
 //если строка не €вл€етс€ таким числом, то возвращает -1
    int a=0,d=0;
    int correct =true;
...
{//переводит строку, в которой записана дес€тична€ дробь с
 //точкой в качестве разделител€ в соответствующее число
 //в случае, если строка не €вл€етс€ корректной записью
...
{//освобождение пам€ти. Ќадеюсь, утечек не будет...
free(m->vs);
free(m->vts);
free(m->vns);
free(m->vps);
...
int counter(mash *m)
{
/*считает количество вершин, нормалей и т.д., выдел€ет соответствующее
количество пам€ти под массивы этих элементов и присваивает массивам
из mash указатели на эту пам€ть */
char tmp[1000];
m->v_count=0;
m->vt_count=0;
m->vn_count=0;
    ...
    while (read_next_str(tmp))
{
    if  (starts_with(tmp,"vp "))
        ...
        {
            *cur_vn=read_vn_from_string(tmp);
            cur_vn++;
